...
// gcc disable_ddio.c -o disable_ddio -lpci
...
struct pci_dev *find_ddio_device(uint8_t nic_bus) {
  ...
  for (dev = pacc->devices; dev; dev = dev->next) {
     ...
     * different
     */
    if (dev->bus == nic_bus /* dev->func == 2 && dev->dev == 0  && */ /* pci_read_byte(
            dev, PCI_SUBORDINATE_BUS) ==
        nic_bus*/ /*&& dev->numa_node==1 && dev->domain==0x10001*/) {
      //		if(dev->func == 0 && dev->bus == 0x3b /*&&
      //dev->numa_node==1 && dev->domain==0x10001*/) {
      printf("func %d\n", dev->func);
      return dev;
    }
  }
...
int main(void) {
  ...
  uint8_t nic_bus = 0x31, ddio_state = 0;
